...
void MeshFlow::Initialize(){
	...
	indexX = cv::Mat::zeros(m_height,m_width,CV_32FC1);
	indexY = cv::Mat::zeros(m_height,m_width,CV_32FC1);
	for(int i=0;i<m_height;i++){
		for(int j=0;j<m_width;j++){
			indexX.at<float>(i,j) = j;   
			indexY.at<float>(i,j) = i;
		}
	}

	X.create(m_quadHeight,m_quadWidth,CV_32FC1);
	Y.create(m_quadHeight,m_quadWidth,CV_32FC1);
	W.create(m_quadHeight,m_quadWidth,CV_32FC1);
	dstX.create(m_quadHeight,m_quadWidth,CV_32FC1);
	dstY.create(m_quadHeight,m_quadWidth,CV_32FC1);
	Tx.create(m_quadHeight,m_quadWidth,CV_32FC1);
	Ty.create(m_quadHeight,m_quadWidth,CV_32FC1);
}
...
void MeshFlow::GetMotions(cv::Mat &mapX, cv::Mat &mapY){
	...
	for (int i = 1; i < m_mesh->height; i++)
	{
		for (int j = 1; j < m_mesh->width; j++)
		{
			...
			cv::Rect rect(source[0].x,source[0].y,(source[3].x-source[0].x),(source[3].y-source[0].y));
			dstX = indexX(rect);
			dstY = indexY(rect);
			X = H.at<double>(0, 0) * dstX + H.at<double>(0, 1) * dstY + H.at<double>(0, 2);
			Y = H.at<double>(1, 0) * dstX + H.at<double>(1, 1) * dstY + H.at<double>(1, 2);
			W = H.at<double>(2, 0) * dstX + H.at<double>(2, 1) * dstY + H.at<double>(2, 2);
			cv::divide(X,W,Tx);
			cv::divide(Y,W,Ty);
			Tx = Tx - dstX;
			Ty = Ty - dstY;
